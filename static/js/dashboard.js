...
function initDashboard() {
  ...
  if (refreshButton) {
    refreshButton.addEventListener('click', () => {
      refreshCurrentPage();
      showToast('Tableau de bord actualisé.', 'success', 1800);
    });
  }
}
...
function afficherInfosUtilisateur(user) {
  ...
  // Sidebar — profil
  setAvatarElement(document.getElementById('sidebarAvatar'), user);
  ...
  setAvatarElement(document.getElementById('profileQuickAvatar'), user);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (page === 'dashboard.html') {
    initDashboard();
    window.addEventListener('annonces:refresh', () => {
      const user = getCurrentUser();
      if (user) {
        afficherInfosUtilisateur(user);
        afficherStatsDashboard(user);
