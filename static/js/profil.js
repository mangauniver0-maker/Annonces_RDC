...
function afficherInfosProfil(user) {
  ...
  if (avatarEl) setAvatarElement(avatarEl, user);
  ...
  if (fullUser && fullUser.createdAt) {
    ...
    function initProfilePhoto(currentUser) {
      ...
      input.addEventListener('change', () => {
        ...
        reader.addEventListener('load', () => {
          ...
          if (index === -1 || typeof reader.result !== 'string') {
            showToast('Impossible de mettre à jour la photo.', 'error');
            return;
          }
          users[index].avatar = reader.result;
          saveToStorage(STORAGE_KEYS.USERS, users);
          const updatedSession = { ...getCurrentUser(), avatar: reader.result };
          saveToStorage(STORAGE_KEYS.CURRENT_USER, updatedSession);
          afficherInfosProfil(updatedSession);
          showToast('Photo de profil actualisée.', 'success');
        });
        reader.addEventListener('error', () => showToast('Lecture de la photo impossible.', 'error'));
...
function handleEditProfil(e, currentUser) {
  ...
  setTimeout(() => {
    ...
    const updatedSession = {
      ...
      avatar: users[userIndex].avatar || currentUser.avatar || '',
      role: users[userIndex].role || currentUser.role || 'student',
    };
    saveToStorage(STORAGE_KEYS.CURRENT_USER, updatedSession);
