...
const STORAGE_KEYS = {
  ...
  ADMIN_LOGS: 'annonces_rdc_admin_logs',
};
...
function normalizeListing(listing) {
  return {
    ...
    auteurId: String(listing.author.id),
    auteurPrenom: listing.author.firstName,
    auteurNom: listing.author.lastName,
    auteurUniversite: listing.author.university || '',
...
async function loadAnnonces() {
  ...
  annoncesCache = data.listings.map(normalizeListing);
  saveAnnonces(annoncesCache);
  return annoncesCache;
}
...
/**
 * Affiche un avatar photo ou les initiales dans un élément.
 */
function setAvatarElement(element, user, className = '') {
  if (!element || !user) return;
  element.classList.toggle('has-photo', Boolean(user.avatar));
  if (className) element.classList.add(className);
  if (user.avatar) {
    element.innerHTML = `<img src="${escapeHtml(user.avatar)}" alt="Photo de ${escapeHtml(user.prenom)}" />`;
  } else {
    element.textContent = getInitials(user.prenom, user.nom);
  }
}
...
function refreshCurrentPage() {
  window.dispatchEvent(new CustomEvent('annonces:refresh'));
}
...
function initDemoData() {
  ...
  if (storedVersion === DEMO_DATA_VERSION) {
    ...
    if (users.some(user => !user.role)) {
      saveToStorage(STORAGE_KEYS.USERS, users.map((user, index) => ({
        ...user,
    ...
    <div class="annonce-card animate-slideUp" data-id="${escapeHtml(annonce.id)}">
      ...
      <div class="annonce-card-footer">
        <div class="annonce-author">
          <div class="author-avatar">${userAvatarMarkup(getUserById(annonce.auteurId), initials)}</div>
...
function userAvatarMarkup(user, fallbackInitials) {
  if (!user || !user.avatar) return escapeHtml(fallbackInitials);
  return `<img src="${escapeHtml(user.avatar)}" alt="Photo de ${escapeHtml(user.prenom)}" />`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  logoutBtns.forEach(btn => {
    ...
    // Synchronise automatiquement plusieurs onglets ouverts.
    window.addEventListener('storage', event => {
      if (Object.values(STORAGE_KEYS).includes(event.key)) refreshCurrentPage();
    });
  });
});
