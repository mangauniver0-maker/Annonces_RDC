 ...
 */
function chargerAnnonceEnEdition(id) {
  const annonces = getAllAnnonces();
  const annonce = annonces.find(a => a.id === id);
  const user = getCurrentUser();
 ...
 */
function handlePublishSubmit(e, editId = null) {
  e.preventDefault();
  ...
  setTimeout(() => {
    ...
    if (editId) {
      // Mode modification
      const index = annonces.findIndex(a => a.id === editId);
      if (index !== -1) {
        annonces[index] = {
          ...annonces[index],
      ...
      const newAnnonce = {
        ...
        auteurNom: user.nom,
        auteurUniversite: user.universite,
        createdAt: new Date().toISOString(),
      };

      annonces.unshift(newAnnonce);  // Ajouter en premier
      saveAnnonces(annonces);
...
function supprimerAnnonce(id) {
  ...
  if (filtered.length === annonces.length) return false;  // Pas trouvé

  saveAnnonces(filtered);
  return true;
}
